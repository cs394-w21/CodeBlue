...
const ThemeScreen = ({navigation}) => {
   ...
   */
  const Item = ({title, color, description, buttonText}) => (
    <TouchableOpacity
      onPress={() => handlePress({title, color, description, buttonText})}
  ...
  const renderItem = ({item}) => (
    <Item title={item.title} color={item.color} description={item.description} buttonText={item.buttonText} />
  );
  return (
    <ScrollView>
